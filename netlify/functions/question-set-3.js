...
const db = () => client.db("Rating-Interface-Website").collection("question-set-3")
...
export async function GET() {
    try {
        ...
        console.error(error)
        return Response.json({error: "Failed to GET question-set-3", status: 500})
    }
}
...
export async function POST(req) {
    try {
        ...
        console.error(error);
        return Response.json({error: "Failed to POST question-set-3", status: 500})
...
export async function DELETE(_, { params }) {
    try {
        ...
        console.error(error);
        return Response.json({error: "Failed to DELETE question-set-3", status: 500})
    }
}